...
const AREAS = {
  ...
  graftonSpeck: {
    ...
    waypoints: [
      { id: 'rt26-grafton', type: 'Road/Trailhead', name: 'Route 26 / Grafton Notch', mile: 0.0, elevationFt: 1490, lat: 44.5935, lon: -70.9466, detail: 'ME 26 trailhead/parking access. Good field-test start point.' },
      { id: 'eyebrow-jct', type: 'Junction', name: 'Eyebrow Trail junction area', mile: 1.1, elevationFt: 2350, lat: 44.5865, lon: -70.9498, detail: 'Approximate junction area; confirm with blazes/map on trail.', amenities: ['alternate-route'] },
      { id: 'old-speck', type: 'Summit', name: 'Old Speck Mountain', mile: 3.5, elevationFt: 4170, lat: 44.5709, lon: -70.9536, detail: 'Major climb and weather/wind checkpoint.', amenities: ['mountain'] },
      { id: 'mahoosuc-arm', type: 'Mountain', name: 'Mahoosuc Arm', mile: 5.1, elevationFt: 3600, lat: 44.5606, lon: -70.9781, detail: 'Steep approach near Speck Pond; take it slow.', amenities: ['mountain'] },
      { id: 'speck-pond', type: 'Pond/Shelter', name: 'Speck Pond', mile: 6.0, elevationFt: 3400, lat: 44.5637, lon: -70.9734, detail: 'Your near-term destination/test area. Pond/shelter coordinates from OSM/Nominatim.', amenities: ['water', 'camp', 'fishing-reported'] },
...
function geoJsonToGpx(featureCollection, name = 'Map-Pi exported route') {
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="Map-Pi-E Trail Planner" xmlns="http://www.topografix.com/GPX/1/1">\n  <metadata><name>${name}</name></metadata>\n  <trk><name>${name}</name>\n${trksegs}\n  </trk>\n</gpx>\n`;
}
...
const OPEN_METEO_CODES = { 0: 'Clear', 1: 'Mostly clear', 2: 'Partly cloudy', 3: 'Overcast', 45: 'Fog', 48: 'Rime fog', 51: 'Light drizzle', 53: 'Drizzle', 55: 'Heavy drizzle', 61: 'Light rain', 63: 'Rain', 65: 'Heavy rain', 71: 'Light snow', 73: 'Snow', 75: 'Heavy snow', 80: 'Rain showers', 81: 'Showers', 82: 'Heavy showers', 95: 'Thunderstorm' };
function appDeviceId() { const key = 'mapPiE.deviceId'; let id = localStorage.getItem(key); if (!id) { id = uid('device'); localStorage.setItem(key, id); } return id; }
async function saveCloudRecord(kind, payload) { if (!supabase) return { ok: false, message: 'Cloud sync not configured: add VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY in Vercel.' }; const { error } = await supabase.from('map_pi_e_records').insert({ device_id: appDeviceId(), kind, payload }); return error ? { ok: false, message: `Cloud save failed: ${error.message}` } : { ok: true, message: `Saved ${kind} locally + Supabase.` }; }
function forecastTargetHour(forecast, hikeStartAt) { if (!forecast?.hourly?.time?.length) return 0; const target = hikeStartAt ? new Date(hikeStartAt).getTime() : Date.now(); let best = 0, delta = Infinity; forecast.hourly.time.forEach((t, i) => { const d = Math.abs(new Date(t).getTime() - target); if (d < delta) { best = i; delta = d; } }); return best; }
function forecastToWeather(forecast, fallback, hikeStartAt) { const i = forecastTargetHour(forecast, hikeStartAt); const h = forecast?.hourly || {}; const tempF = h.temperature_2m?.[i] ?? fallback.tempF; const humidity = h.relative_humidity_2m?.[i] ?? fallback.humidity; const windMph = h.wind_speed_10m?.[i] ?? fallback.windMph; const precip = h.precipitation_probability?.[i] ?? 0; const code = h.weather_code?.[i]; const elevationGainFt = Math.round((forecast?.elevation || fallback.elevationGainFt || 0) * 3.28084); const base = estimateWeather(null, { ...fallback, tempF, humidity, windMph, airQualityIndex: fallback.airQualityIndex || 35, elevationGainFt, condition: OPEN_METEO_CODES[code] || fallback.condition, source: 'Open-Meteo forecast', updatedAt: new Date().toISOString() }); return { ...base, precipitationProbability: precip, elevationFt: elevationGainFt, forecastTime: h.time?.[i], summary: `${base.condition} · ${Math.round(tempF)}°F · ${precip}% precip · wind ${Math.round(windMph)} mph` }; }
async function fetchOpenMeteoForecast(point) { if (!point?.lat || !point?.lon) return null; const params = new URLSearchParams({ latitude: String(point.lat), longitude: String(point.lon), temperature_unit: 'fahrenheit', wind_speed_unit: 'mph', precipitation_unit: 'inch', timezone: 'auto', forecast_days: '7', hourly: 'temperature_2m,relative_humidity_2m,precipitation_probability,weather_code,wind_speed_10m', daily: 'weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,sunrise,sunset' }); const res = await fetch(`https://api.open-meteo.com/v1/forecast?${params.toString()}`); if (!res.ok) throw new Error(`Open-Meteo HTTP ${res.status}`); return res.json(); }
...
function App() {
  ...
  const [cloudStatus, setCloudStatus] = useState(supabase ? 'Supabase ready for saved routes/markers.' : 'Supabase env missing; saving on this device only.');
  ...
  useEffect(() => { let cancelled = false; fetchOpenMeteoForecast(destination || current).then(data => { if (!cancelled) setForecast(data); }).catch(e => { if (!cancelled) setCloudStatus(`Weather forecast unavailable: ${e.message}`); }); return () => { cancelled = true; }; }, [destination?.lat, destination?.lon, current?.lat, current?.lon]);
  const weather = useMemo(() => forecast ? forecastToWeather(forecast, weatherInput, planner.hikeStartAt) : estimateWeather(current, weatherInput), [forecast, weatherInput, planner.hikeStartAt, current]);
  ...
  const savePlan = async (asNew = false) => { const existing = routePlans.find(r => r.id === activeRoutePlanId); const plan = makeRoutePlan({ planner, area, route: snappedRoute, navigation, visibility: planner.visibility || existing?.visibility || 'private' }); if (!asNew && existing) { plan.id = existing.id; plan.createdAt = existing.createdAt; setRoutePlans(prev => prev.map(r => r.id === existing.id ? plan : r)); setActiveRoutePlanId(existing.id); flashSaved('Route updated and saved on this device.'); saveCloudRecord('route_plan', plan).then(r => setCloudStatus(r.message)); } else { setRoutePlans(prev => [plan, ...prev]); setActiveRoutePlanId(plan.id); flashSaved('New route saved on this device.'); saveCloudRecord('route_plan', plan).then(r => setCloudStatus(r.message)); } };
  ...
  const saveProfile = () => { flashSaved('Profile, settings, and weather saved on this device.'); saveCloudRecord('profile_settings', { settings, userProfile, weatherInput }).then(r => setCloudStatus(r.message)); };
  ...
  const addMarker = (point, name = 'Saved marker') => { const marker = { id: uid('marker'), name, category: 'Scout later', visibility: 'private', lat: Number(point.displayLat || point.lat), lon: Number(point.displayLon || point.lon), note: '', createdAt: new Date().toISOString() }; setSavedMarkers(prev => [marker, ...prev]); flashSaved('Marker saved on this device.'); saveCloudRecord('marker', marker).then(r => setCloudStatus(r.message)); };
  ...
  return <main className={`app-shell flow-shell theme-${settings.theme} ${settings.fieldMode ? 'field-mode' : ''}`}>
    <header className="app-header glass"><div><p className="eyebrow"><MapPin size={16}/> Map-Pi-E trail planning flow</p><h1>Trail Planner</h1><p>{planner.planName || area.label} · {hike.active ? 'Hike active' : 'Plan ready'} · {cloudStatus}</p></div><div className={`status-pill risk-${weather.risk}`}><Wifi size={16}/> Realtime forecast · {weather.summary}</div></header>
...
function RouteMap({ area, route, current, weather, destination, onPick, trailGeometry, sosMarker, navigation, settings = {}, markers = [] }) {
  ...
  useEffect(() => {
    ...
    [...route, ...WATER_REPORTS].filter(item => featureVisible(item, visibility)).forEach((w) => { const lat = w.displayLat || w.lat; const lon = w.displayLon || w.lon; if (!Number.isFinite(Number(lat)) || !Number.isFinite(Number(lon))) return; const isDest = destination?.id === w.id; const icon = L.divIcon({ className: 'map-feature-icon', html: `<span style="--pin:${featureColor(w, isDest ? '#ffdb77' : '#bfe0a9')}">${featureIcon(w)}</span>`, iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }); const marker = L.marker([lat, lon], { icon }); const distanceText = current ? `<br/>Distance from you: ${fmtMiles(haversineMiles(current, { lat, lon }), settings.units)}` : ''; const altitudeText = w.elevationFt ? `<br/>Altitude: ${Math.round(w.elevationFt).toLocaleString()} ft` : (weather?.elevationFt ? `<br/>Area elevation: ${Math.round(weather.elevationFt).toLocaleString()} ft` : ''); const weatherText = weather ? `<br/>Weather: ${weather.summary}${weather.forecastTime ? ` @ ${new Date(weather.forecastTime).toLocaleString()}` : ''}` : ''; marker.bindPopup(`<strong>${w.name}</strong><br/>${w.type || 'Marker'}${w.mile != null ? ` · mile ${w.mile}` : ''}${distanceText}${altitudeText}${weatherText}<br/>${w.detail || ''}`); marker.on('click', () => onPick?.(w)); marker.addTo(group); bounds.push([lat, lon]); });
